...
function AccountBox({ user, onNavigate, onLogout }) {
  ...
  return <div className="account-box">
    ...
    {user.role === "admin" && <button className="account-link" onClick={() => onNavigate("admin-jogos")}>Enviar jogo</button>}
...
function Header({ view, onNavigate, search, setSearch, user, onLogout }) {
...
function Hero({ game, onDetails, onMethodology }) {
  ...
  return <section className="hero" style={{ "--hero-image": `url(${game.heroUrl})` }}><div className="hero-content"><div className="spotlight"><span>DATA SPOTLIGHT</span><Circle size={7} weight="fill" color="var(--blue)" /> <time>{today}</time></div><div className="eyebrow">O JOGO DO MOMENTO</div><h1>{game.shortTitle || game.title}</h1><p>{game.summary}</p><div className="hero-actions"><button className="primary-button" onClick={onDetails}>Ver detalhes</button><button className="list-button"><BookmarkSimple size={19} /> Adicionar à lista</button></div></div><aside className="hero-score"><div className="score-heading"><h2>Índice SteamTwo</h2><button onClick={onMethodology}>Como calculamos <Info size={15} /></button></div><div><Score value={game.score} large /> <span className="out-of">/100</span></div><p>Pontuação e catálogo vêm inteiramente do IGDB — a mesma fonte da capa, dos gêneros e da popularidade.</p><div className="score-lines"><span><TrendUp size={19} /> Popularidade <b>{formatScore(game.score)}</b></span><span><Star size={19} /> Avaliação IGDB <b>{game.qualityScore != null ? formatScore(game.qualityScore) : "—"}</b></span><span><UsersThree size={19} /> Hype <b>{formatHype(game.hypeCount)}</b></span></div><small>Atualizado via sincronização do IGDB</small></aside></section>;
}
...
function IndependentBadge({ compact = false }) { return <span className={`badge-independent ${compact ? "badge-independent-compact" : ""}`}><Sparkle size={12} weight="fill" /> Independente</span>; }
function GameCard({ game, onDetails }) { return <button className="game-card" onClick={() => onDetails(game)}>{game.origin === "admin" && <IndependentBadge compact />}<SafeImage src={game.coverUrl} alt={`Capa de ${game.shortTitle}`} /><div><span className="game-card-score"><Score value={game.score} /></span><h3>{game.shortTitle}</h3><p>{game.genres.join(" · ")}</p><span className="card-stores">{game.stores.map(storeLabel).join(" · ")}</span></div></button>; }
function Catalog({ data, query, onDetails }) {
  ...
  useEffect(() => {
    ...
    if (independentOnly) params.set("independent", "true");
    setLoading(true);
    setApiError(false);
  ...
  }, [query, store, genre, independentOnly]);
  const source = apiGames ?? fallbackGames;
  const genres = [...new Set(source.flatMap((game) => game.genres || []))];
  const filtered = source.filter((game) => (!query || game.title.toLowerCase().includes(query.toLowerCase())) && (store === "all" || game.stores.includes(store)) && (genre === "all" || game.genres.includes(genre)) && (!independentOnly || game.origin === "admin"));
  return <section className="catalog-page"><div className="page-intro"><div><span className="eyebrow">BIBLIOTECA STEAMTWO</span><h1>Catálogo de jogos</h1><p>Explore o catálogo do SteamTwo com dados do IGDB — links de compra continuam apontando para Steam e Epic Games quando disponíveis.</p></div><GameController size={58} weight="thin" /></div><div className="catalog-toolbar"><div className="filter-control"><FunnelSimple size={18} /><StoreFilters value={store} onChange={setStore} /></div><label className="select-control">Gênero <select value={genre} onChange={(event) => setGenre(event.target.value)}><option value="all">Todos os gêneros</option>{genres.map((item) => <option value={item} key={item}>{item}</option>)}</select><CaretDown size={16} /></label><button type="button" className={`independent-filter ${independentOnly ? "selected" : ""}`} onClick={() => setIndependentOnly((value) => !value)} aria-pressed={independentOnly}><Sparkle size={16} weight={independentOnly ? "fill" : "regular"} /> Independentes</button></div>{loading ? <div className="empty-state"><GameController size={34} /><h2>Carregando catálogo</h2><p>Buscando jogos nas fontes do SteamTwo…</p></div> : filtered.length ? <div className="catalog-grid">{filtered.map((game) => <GameCard game={game} onDetails={onDetails} key={game.id} />)}</div> : <div className="empty-state"><GameController size={34} /><h2>{apiError ? "Catálogo temporariamente indisponível" : "Nenhum jogo encontrado"}</h2><p>{apiError ? "Exibindo os últimos jogos válidos." : "Tente outra busca ou remova algum filtro."}</p></div>}</section>;
}
...
function Detail({ game, onBack, onMethodology }) { return <section className="detail-page"><button className="back-link" onClick={onBack}><ArrowRight size={17} /> Voltar</button><div className="detail-hero" style={{ "--detail-image": `url(${game.heroUrl})` }}><div><span className="eyebrow">DETALHE DO JOGO</span><h1>{game.shortTitle}</h1>{game.origin === "admin" && <IndependentBadge />}<div className="detail-tags">{game.genres.map((genre) => <span key={genre}>{genre}</span>)}</div><p>{game.summary}</p><div className="detail-actions"><a className="primary-button" href={storeUrlForGame(game)} target="_blank" rel="noreferrer">Abrir na loja <ArrowSquareOut size={17} /></a><button className="list-button"><BookmarkSimple size={19} /> Adicionar à lista</button></div></div><div className="detail-score"><span>ÍNDICE STEAMTWO</span><Score value={game.score} large /><small>/ 100</small><button onClick={onMethodology}>Como calculamos <Info size={14} /></button></div></div><div className="detail-columns"><article><h2>Sobre este jogo</h2><p>{game.summary} O índice vem da popularidade do IGDB, para oferecer uma leitura comparável entre os títulos.</p></article><aside><h2>Fontes</h2><div className="source-pills">{game.stores.map((store) => <span key={store}><Check size={14} /> {storeLabel(store)}</span>)}<span><Check size={14} /> IGDB</span></div><small>Última atualização: 24 de ago. de 2026</small></aside></div></section>; }
function Methodology({ onClose }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="methodology-modal" role="dialog" aria-modal="true" aria-labelledby="method-title"><button ref={closeRef} className="modal-close" aria-label="Fechar" onClick={onClose}><X size={20} /></button><span className="eyebrow">TRANSPARÊNCIA</span><h2 id="method-title">Como calculamos</h2><p>O Índice SteamTwo vem inteiramente da popularidade do IGDB, para destacar jogos em alta sem fingir que a plataforma recebe jogadores.</p><div className="method-grid"><div><strong>01</strong><h3>Popularidade IGDB</h3><p>A pontuação é a popularidade normalizada que o IGDB calcula para cada jogo, atualizada pela sincronização do catálogo.</p></div><div><strong>02</strong><h3>Catálogo</h3><p>Título, capa, gêneros e data de lançamento também vêm do IGDB — a mesma fonte da pontuação.</p></div><div><strong>03</strong><h3>Links de loja</h3><p>Quando o IGDB identifica o jogo na Steam ou na Epic Games, mostramos o link de compra — mas isso não influencia a pontuação.</p></div></div><small className="method-foot">Fonte: IGDB · dados de 18–24 de agosto de 2026</small></section></div>;
}
export function App() {
